...
#if !defined(KRATOS_RANS_RESIDUALBASED_NEWTON_RAPHSON_STRATEGY)
#define KRATOS_RANS_RESIDUALBASED_NEWTON_RAPHSON_STRATEGY
...
/**
 * @class RansResidualBasedNewtonRaphsonStrategy
 * @ingroup KratosCore
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(RansResidualBasedNewtonRaphsonStrategy);
     ...
     */
    RansResidualBasedNewtonRaphsonStrategy(ModelPart& rModelPart,
          ...
          mKeepSystemConstantDuringIterations(false)
    {
        ...
        // Tells to the Builder And Solver if the system matrix and vectors need
        // to be reshaped at each step or not
     ...
     */
    ~RansResidualBasedNewtonRaphsonStrategy() override
    {
...
#pragma omp parallel for
            ...
            // The following is needed since we need to eventually compute time
            // derivatives after applying Master slave relations
    ...
    void Initialize() override
    {
        ...
        if (mInitializeWasPerformed == false)
        {
            ...
            typename TConvergenceCriteriaType::Pointer p_convergence_criteria =
    ...
    bool IsConverged() override
    {
        ...
        return (mpConvergenceCriteria->PostCriteria(
                    BaseType::GetModelPart(), GetBuilderAndSolver()->GetDofSet(), rA, rDx, rb) &&
                !BaseType::GetModelPart().GetProcessInfo()[IS_RAMPING]);
    ...
    void InitializeSolutionStep() override
    {
        ...
        if (!mSolutionStepIsInitialized)
        {
            ...
            // set up the system, operation performed just once unless it is
            // required to reform the dof set at each iteration
                ...
                mReformDofSetAtEachStep == true)
            {
                ...
                p_builder_and_solver->ResizeAndInitializeVectors(
                    p_scheme, mpA, mpDx, mpb, r_model_part);
    ...
    void FinalizeSolutionStep() override
    {
        ...
        mpConvergenceCriteria->FinalizeSolutionStep(
            r_model_part, p_builder_and_solver->GetDofSet(), rA, rDx, rb);
    ...
    bool SolveSolutionStep() override
    {
        ...
        mpConvergenceCriteria->InitializeNonLinearIteration(
            r_model_part, r_dof_set, rA, rDx, rb);
        is_converged =
            mpConvergenceCriteria->PreCriteria(r_model_part, r_dof_set, rA, rDx, rb);
        ...
        mpConvergenceCriteria->FinalizeNonLinearIteration(
            r_model_part, r_dof_set, rA, rDx, rb);
        ...
        if (is_converged)
        {
            ...
            is_converged =
                (mpConvergenceCriteria->PostCriteria(r_model_part, r_dof_set, rA, rDx, rb) &&
                 !BaseType::GetModelPart().GetProcessInfo()[IS_RAMPING]);
        }
        ...
        while (is_converged == false && iteration_number++ < mMaxIterationNumber)
        {
            ...
            is_converged = mpConvergenceCriteria->PreCriteria(
                r_model_part, r_dof_set, rA, rDx, rb);
            ...
            if (SparseSpaceType::Size(rDx) != 0)
            {
                if (BaseType::mRebuildLevel > 1 || BaseType::mStiffnessMatrixIsBuilt == false)
                {
                    if (GetKeepSystemConstantDuringIterations() == false)
                    {
                        ...
                        p_builder_and_solver->BuildAndSolve(
                            p_scheme, r_model_part, rA, rDx, rb);
                    }
                    else
                    {
                        TSparseSpace::SetToZero(rDx);
                        ...
                        p_builder_and_solver->BuildRHSAndSolve(
                            p_scheme, r_model_part, rA, rDx, rb);
                    }
                }
                else
            ...
            if (is_converged == true)
            {
                ...
                is_converged = (mpConvergenceCriteria->PostCriteria(
                                    r_model_part, r_dof_set, rA, rDx, rb) &&
                                !BaseType::GetModelPart().GetProcessInfo()[IS_RAMPING]);
            }
        }
        ...
        if (residual_is_updated == false)
        {
            ...
            // part please notify the community via mailing list before
            // uncommenting it. Pooyan.
    ...
    {
        return "RansResidualBasedNewtonRaphsonStrategy";
    }
...
protected:
    ...
    * @details Default = false
    ...
    virtual void UpdateDatabase(TSystemMatrixType& rA,
                                ...
                                const bool MoveMesh)
    {
        ...
        p_scheme->Update(BaseType::GetModelPart(),
                         p_builder_and_solver->GetDofSet(), rA, rDx, rb);
    ...
    virtual void EchoInfo(const unsigned int IterationNumber)
    {
        ...
        else if (this->GetEchoLevel() == 4) // print to matrix market file
        {
            ...
            matrix_market_name
                << "A_" << BaseType::GetModelPart().GetProcessInfo()[TIME]
                << "_" << IterationNumber << ".mm";
            TSparseSpace::WriteMatrixMarketMatrix(
                (char*)(matrix_market_name.str()).c_str(), rA, false);

            std::stringstream matrix_market_vectname;
            matrix_market_vectname
                << "b_" << BaseType::GetModelPart().GetProcessInfo()[TIME]
                << "_" << IterationNumber << ".mm.rhs";
            TSparseSpace::WriteMatrixMarketVector(
                (char*)(matrix_market_vectname.str()).c_str(), rb);
        }
    }
    ...
    RansResidualBasedNewtonRaphsonStrategy(const RansResidualBasedNewtonRaphsonStrategy& Other){};
...
#endif /* KRATOS_RANS_RESIDUALBASED_NEWTON_RAPHSON_STRATEGY  defined */
