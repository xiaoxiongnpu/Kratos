...
RansEvmKOmegaKElement<TDim, TNumNodes>::RansEvmKOmegaKElement(
    RansEvmKOmegaKElement<TDim, TNumNodes> const& rOther)
    ...
    const int Step) const
{
    ...
    //additional term of kinetic energy of the previous time step, used in calculation of the k element production term, figure out why it is not working
												...
												const int Step) const
{
    ...
    return std::max(rData.BetaStar * rData.Theta * rData.TurbulentKineticEnergy + (2.0 / 3.0) *rData.Gamma* rData.VelocityDivergence, 0.0);
}
...
template class KRATOS_API(RANS_APPLICATION) RansEvmKOmegaKElement<2, 3>;
template class KRATOS_API(RANS_APPLICATION) RansEvmKOmegaKElement<3, 4>;
template class KRATOS_API(RANS_APPLICATION) RansEvmKOmegaKElement<2, 4>;
template class KRATOS_API(RANS_APPLICATION) RansEvmKOmegaKElement<3, 8>;
