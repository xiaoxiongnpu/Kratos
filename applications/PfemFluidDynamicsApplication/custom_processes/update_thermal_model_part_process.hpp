...
public:
    ...
    /// Constructor
    UpdateThermalModelPartProcess(ModelPart& origin_model_part,
        ModelPart& destination_model_part,
        ModelPart& computing_model_part,
            ...
            if (i_mp->NumberOfElements()) {
                ...
                if ((i_mp->Is(SOLID) && i_mp->IsNot(ACTIVE)) || (i_mp->Is(FLUID) && i_mp->IsNot(ACTIVE)) || (i_mp->Is(BOUNDARY) && i_mp->Is(RIGID))) {
                    ...
                    for (auto it_elem = i_mp->ElementsBegin(); it_elem != i_mp->ElementsEnd(); ++it_elem) {
                        ...
                        Element::Pointer p_element = mReferenceElement.Create(it_elem->Id(), it_elem->GetGeometry(), properties);
